...
// +------------------------------------------------------------+
	...
	if(isset($_SESSION[$GLOBALS['APP']."USERID"]))
		...
		debug("exist","warning","session empty userid value");
	...
	if(!$res){
		$answer['results'] = false;
		debug("exist","warning","mysql error: ".mysqli_error($conn));
		echo json_encode($answer);
		return;
	}
	...
	if(!$row){
		$answer['results'] = false;
		debug("exist","error","friends research: can't find requesting user -$userid- in DB with status ID = 1, this might be a session issue or a bad request");
		echo json_encode($answer);
		return;
	}
	...
	if($name!=""){	
		$sql = "SELECT USER_S,NAME_S, DATA_S->'$.\"private-data\".backgroundPicture' as BACKGROUND, PICTURES->'$.\"profilePicture\"' as PICTURE, DATA_S->'$.\"pending_invites\"' as PENDING, DATA_S->'$.\"private-data\".topSong' as TOPSONG, DATA_S->'$.\"private-data\".skills' as SKILL, DATA_S->'$.\"private-data\".location.latitude' as LATITUDE, DATA_S->'$.\"private-data\".location.longitude' as LONGITUDE, DATA_S->'$.\"private-data\".isPrivate' as PRIVATE FROM LIFISTIC_1.LIFISTIC_USERS WHERE $filters NAME_S COLLATE LATIN1_GENERAL_CI LIKE '%$name%' AND STATUS_ID = 1 LIMIT 500"; 
	}else{
		$sql = "SELECT USER_S,NAME_S, DATA_S->'$.\"private-data\".backgroundPicture' as BACKGROUND, PICTURES->'$.\"profilePicture\"' as PICTURE, DATA_S->'$.\"pending_invites\"' as PENDING, DATA_S->'$.\"private-data\".topSong' as TOPSONG, DATA_S->'$.\"private-data\".skills' as SKILL, DATA_S->'$.\"private-data\".location.latitude' as LATITUDE, DATA_S->'$.\"private-data\".location.longitude' as LONGITUDE, DATA_S->'$.\"private-data\".isPrivate' as PRIVATE FROM LIFISTIC_1.LIFISTIC_USERS WHERE $filters STATUS_ID = 1 LIMIT 500"; 
	}	
